...
class Flappy():
    ...
    def get_orientation(self):      
        ...
        pitch = np.asin(np.clip(-R_body.T[0,2],-1,1))
        yaw = np.atan2(R_body.T[0,1], R_body.T[0,0])
        roll = np.atan2(R_body.T[1, 2], R_body.T[2, 2])
    ...
    def step(self, u_gain):
        ...
        # tok2 = time.time()
        fk3, fd3, fa3 = func_eom(self.xk + fk2 * self.dt / 2, self.xd + fd2 * p.dt / 2, self.xa + fa2 * self.dt / 2, u1, f_thruster,t_thruster, self.p, yaw_damping)
        # tok3 = time.time()

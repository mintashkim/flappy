...
class FlappyEnv(MujocoEnv, utils.EzPickle):
    ...
    def _get_reward(self, action, obs_curr):
        ...
        if self.is_bonus and self.trajectory_type == "linear":
            ...
            bonus = 0.1*(1-np.exp(-np.min([np.abs(int(current_pos[0])), goal_pos_x])))
            if np.linalg.norm(current_pos - self.bonus_point[np.min([np.abs(int(current_pos[0])), goal_pos_x])]) < 0.1: 
                total_reward += bonus
                if bonus > 0:
                    print("Bonus earned  |  Bonus Point: {bp}  |  Postion: {pos}  |  Bonus: {bonus}".format(
                          bp=self.bonus_point[np.min([np.abs(int(current_pos[0])), goal_pos_x])],
